...
<div class="grid"><section class="card"><h2>Train + held-out evaluate</h2><div class="muted">Training uses only the train split. Validation/test remains held out.</div><label>Dataset ID</label><input id="trainDataset" placeholder="ds_..."><div class="row"><div><label>Evaluate on</label><select id="evalSplit"><option>validation</option><option>test</option><option>none</option></select></div><div><label>Device</label><select id="device"><option>auto</option><option>cpu</option><option>cuda</option></select></div></div><button onclick="trainDataset()">Train model</button><pre class="result" id="trainResult"></pre></section>
...
async function createDataset(){try{const b={name:name.value,decision_id:decision.value,source_type:source.value,provenance:provenance.value,rights_attested:true,contains_personal_data:pii.value==='true'};const d=await api('/v1/datasets',{method:'POST',body:JSON.stringify(b)});show('createResult',d);datasetId.value=d.id;trainDataset.value=d.id;loadDatasets()}catch(e){show('createResult',{error:e.message})}}
async function addExamples(){try{show('exampleResult',await api('/v1/datasets/'+datasetId.value+'/examples',{method:'POST',body:JSON.stringify({examples:JSON.parse(examples.value)})}))}catch(e){show('exampleResult',{error:e.message})}}
async function importReviews(){try{show('exampleResult',await api('/v1/datasets/'+datasetId.value+'/import-reviews',{method:'POST',body:JSON.stringify({split:'train',limit:5000})}))}catch(e){show('exampleResult',{error:e.message})}}
async function trainDataset(){try{show('trainResult',{status:'training...'});show('trainResult',await api('/v1/datasets/'+trainDataset.value+'/train',{method:'POST',body:JSON.stringify({device:device.value,evaluate_on:evalSplit.value})}))}catch(e){show('trainResult',{error:e.message})}}
async function loadCandidates(){try{show('candidateResult',await api('/v1/active-learning/candidates?decision_id='+encodeURIComponent(activeDecision.value)+'&limit=100'))}catch(e){show('candidateResult',{error:e.message})}}
